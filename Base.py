...
class Reader:
  ...
  def read(self,name):
    # 读取指定名称的CSV文件，路径为self.path/name.csv
    csv_path = os.path.join(self.path, name + '.csv')
    # 使用pandas读取CSV文件，假设无表头（header=None）
    try:
        # 优先尝试使用gbk编码（常见中文Windows系统编码）
        df = pd.read_csv(csv_path, header=None, encoding='gbk')
    except UnicodeDecodeError:
        try:
            # gbk失败时尝试gb18030（gbk扩展编码，覆盖更多中文字符）
            df = pd.read_csv(csv_path, header=None, encoding='gb18030')
        except UnicodeDecodeError:
            # 最后尝试utf-8-sig（支持带BOM的UTF-8文件）
            df = pd.read_csv(csv_path, header=None, encoding='utf-8-sig')
    # 将DataFrame行转换为元组迭代器（去除索引列）
    self.iter = df.itertuples(index=False, name=None)
    return self
  ...
  def to_dict(self,_map=lambda x: (x[0], x[1]),_filter=None):
...
SS_INVALID=reader.read('SS_INVALID').to_dict()
